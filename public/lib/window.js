//This is fired every time the proxy is loaded (aka when a url is requested)

let combo = ["Dev", ""]

document.addEventListener('keydown', function(key){
  if (combo[1].length === combo[0].length){
    combo[1] = key.key
  } else{
    combo[1] = combo[1] + key.key
  }
  
  if (combo[1].toLowerCase() == combo[0].toLowerCase()){
